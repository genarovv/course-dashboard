...
| archived_at | datetime, null | Дата архивации (C2, ARCHITECTURE v2 §6). При `IS NOT NULL` репозиторий исключается из обходов и матрицы, но его снапшоты и вердикты сохраняются (FR-9). Ошибочный URL архивируется, не удаляется. **Путь записи (D45, #71):** реконсиляция при импорте CSV — чего нет в файле, уходит в архив; что вернулось в файл, разархивируется. Отдельных маршрутов архивации нет by design: реестр правится файлом. |
...
| content_probes | json, null | T2 (#44): пробы содержимого `[{key, label, contains?, not_contains?}]` — только объявленные требования курса; часть конфига (реконсиляция из config.yaml) |
...
### BranchHint — работа вне основной ветки (D43, #69)

Журнальная сущность, append-only: строка фиксирует «как было на момент обхода». Ветки переписывают и удаляют, выводов из их истории не делаем.

| Поле | Тип | Смысл |
|------|-----|-------|
| sync_run_id | FK SyncRun | обход, в котором подсказка получена |
| repository_id | FK Repository | репозиторий |
| branch_name | string | имя ветки-кандидата |
| head_sha | string(40) | голова ветки на момент обхода |
| head_date | datetime, null | дата головного коммита; сбой чтения не отменяет подсказку (как D19) |
| artifacts_found | int | ролей курса в дереве этой ветки |
| artifacts_in_default | int | ролей курса в основной ветке — для сравнения в интерфейсе |

Уникальность: `(sync_run_id, repository_id, branch_name)` — одна подсказка на ветку за обход.

**Вердикты по этим веткам не считаются.** Канон связности остаётся за основной веткой; подсказка существует, чтобы пустая ячейка матрицы не читалась как «студент ничего не сделал», когда работа лежит в ветке, слить которую он не может.
...
| points | json | ≤5 точек: `[{entity, quote, why}]` — канон ключей (решение CEO 2026-07-30, ревью C2: ровно эти ключи рождает промпт клиента и читают карточка/evidence_chain) |
...
### 1.14 MrObservation — наблюдение запроса на слияние (добавлена 2026-07-28: FR-12, ADR-007)

Порядок сдачи занятия 11 перевёл сдачу в канал MR — наблюдения файлов default-ветки перестало хватать. `MrObservation` — append-only журнал (как `ArtifactSnapshot`): `id`, `sync_run_id` (FK), `repository_id` (FK), `mr_number` (iid GitLab / number GitHub), `title`, `source_branch`, `state` (opened | merged | closed), `reviewer_approved` (bool — вердикт «принято» агента-ревьюера в обсуждении; отрицание не считается), `markers` (JSON: ключ маркера из конфига → найден/нет + цитата строки), `head_sha` (голова ветки MR на момент наблюдения), `updated_at` (у хостинга), `observed_at`. Не привязан ни к занятию, ни к роли артефакта — это наблюдение процесса, не документа (обоснование новой сущности — ADR-007). Маркеры задаются конфигом FR-2 (`process_markers`), их изменение не требует миграции.
...
- **И12 (единственность наблюдения MR, 2026-07-28):** не более одной строки `mr_observation` на тройку `(sync_run_id, repository_id, mr_number)` — FR-12, ADR-007.
