...
## Дополнение 2026-07-30 (пост-фактум ревью + тикеты #48/#50)

Эфирный коммит 14e68fd прошёл пост-фактум ревью (`reviews/ревью-пост-фактум-ADR-006-2026-07-30.md`, APPROVE с замечаниями). Исправления:
- **#48:** у пустого проекта GitLab отдаёт `default_branch: null` — раньше это роняло весь обход (TypeError/IntegrityError); теперь None не пишется, репозиторий уходит в `repo_unavailable`.
- **#50:** мутация `repo.default_branch` узаконена через `store.update_repository_default_branch` (5-й узкий `update_*`, §3.5) — прямые присваивания в сервисах были нарушением контракта store и запрещены тестом-ограничителем; дублировавшийся блок детекта собран в `services/branch_detect.py`.
- Примечание: сообщение коммита 14e68fd ошибочно ссылается на «(#6)» (тикет S4 config_manager) — фикс эфира к нему отношения не имеет.
