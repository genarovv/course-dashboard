...
> - Раздел «журнал vs состояние» — честный контракт store.py: `register_*` + 5 узких `update_*` + пара archive/restore (§3.5; 5-й добавлен тикетом #50, пара — #71)
...
| D7 | Ролей артефакта | 15 (enum; 9 базовых + 6 пакета «12 артефактов», решение CEO 2026-07-30) | DM §1.4 |
| D8 | Рёбер связности (макс.) | 9 (8 конвейера + prd→architecture, решение CEO 2026-07-30) | PRD §12; DM §1.5 |
...
| ORM | SQLAlchemy 2.x (Mapped) | D18: журнальные сущности — insert + select; мутации — только по трёхчастному контракту §3.5 (состояние — 5 узких `update_*` + пара archive/restore, конфиг-реконсиляция — только config_manager). Unit-of-work и change-tracking не используются — легковесный mapper: Mapped + session.add() / session.execute(select()) |
...
| Риск | Снижение |
|------|----------|
| SQLITE_BUSY при конкурентной записи | Единый writer: обход репозиториев последователен; LLM-задачи ставятся сводом последовательно. Второй обход поверх идущего не начинается: `is_sync_running` → 409, кнопка «обновить сейчас» погашена (D42) |
...
│
├── store.py                 # Data access: register_* (журнал) + 5 узких update_* + archive/restore + конфиг-реконсиляция (§3.5) + select
│
├── services/
...
- `coherence_analyzer.py` — гейт Фазы 0 **пройден 2026-07-30** (мини-эвал C3: golden set 2/2, ADR-004 Accepted, решение CEO): ядро реализовано (C2/#36) и подключено к своду через `make_verdict_worker` (роут /sync); без ключа DeepSeek воркер = None, пары честно остаются «проверяется».
...
- **store.py** — единая точка доступа к данным. Экспортирует `register_*` / `find_*`, ровно 5 узких `update_*`, пару `archive_repository`/`restore_repository` и функции конфиг-реконсиляции (вызывает только config_manager) — §3.5. Для журнальных сущностей update/delete физически не экспортируются. Внутри — прямые SQLAlchemy-запросы.
...
- **Логирование (#75)** — единая конфигурация в `logging_config.py`, подключается при старте `main.py`. Каждая запись: UTC-время, уровень, модуль-источник. Ошибки — с контекстом и трассировкой; ПД/пароли/токены не пишутся (NFR-3), оператор в логе — короткий хеш `user_marker`. Логгеры uvicorn (включая access) — в том же формате: их собственные хендлеры снимаются, записи идут через root (D47); текст access-сообщения uvicorn не переделывается.
...
| `archive_repository` | `Repository.archived_at` ← now | реконсиляция реестра из CSV (D45, #71): репозиторий исчез из файла — уходит из обходов и матрицы, история наблюдений цела (C2, FR-9) |
| `restore_repository` | `Repository.archived_at` ← NULL | возврат репозитория в реестр (D45, #71); удаления нет — мягкое гашение, как у Override |

Других мутаций состояния не появляется (поверхность закрыта тестом-ограничителем, #73); `delete_*` нет вообще.
...
В DM §3.3 инварианты И1–И12 объявлены текстом. Здесь каждый заземлён на физический механизм — с градацией соразмерности: **DDL там, где constraint дёшев и ловит реальную порчу; код/сид там, где DDL невозможен или несоразмерен** (single-user, single-writer).
...
POST /sync  →  sync_orchestrator.run_sync()
  ...
  → session.commit()                              # ОБЯЗАТЕЛЬНО до свода (FIX-I2):
      ...
      # им не видны, пара со свежим снапшотом падает в И2 (боевой случай 04.08)
...
| ячейка MR-занятия без артефактов (`submission_channel=mr`, статус пустой/not_found) | «сдача через MR, не наблюдается» + ссылка на карточку — не «нет» | FR-12 интерим, ADR-007 |
