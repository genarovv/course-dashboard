...
| ORM | SQLAlchemy 2.x (Mapped) | D18: журнальные сущности — insert + select; состояние — 4 узких `update_*` (§3.5). Unit-of-work и change-tracking не используются — легковесный mapper: Mapped + session.add() / session.execute(select()) |
...
│
├── store.py                 # Data access: register_* (журнал) + 4 узких update_* (§3.5) + select
│
├── services/
...
- **store.py** — единая точка доступа к данным. Экспортирует `register_*` / `find_*` и ровно 4 узких `update_*` (§3.5). Для журнальных сущностей update/delete физически не экспортируются. Внутри — прямые SQLAlchemy-запросы.
...
### 3.5 Журнал vs состояние — честный контракт store.py (v3)

Контракт v2 «store.py — только insert/select» был фактически неверен: UPDATE нужен пяти местам (находка 2 ревью). Вместо того чтобы прятать мутации, разделяем явно:

**Журнал (иммутабельно, только `register_*`):** `Rubric`, `ArtifactSnapshot`, `CoherenceVerdict`, `SyncRunRepository`, создание `Override`. Для них store.py не экспортирует update/delete — это и есть основной механизм И5 (плюс триггеры на 2 доказательных таблицах, §4).

**Состояние (ровно 4 узких `update_*`):**
...
Других `update_*` не появляется; `delete_*` нет вообще.
