...
| `pre-push` | `ruff check .` + полная suite с coverage-гейтом: `pytest --cov=app`, порог `fail_under = 90` (pyproject) |
...
Первая итерация кода в работе (занятия 9–13): модели + миграции, store, аутентификация, CSV-импорт, git_client, config_manager (FR-2), обход репозиториев с детектом заготовок (FR-8/FR-4/BR-3), свод-реконсиляция LLM-пар (без ядра), матрица (FR-4), карточка студента (FR-9), Override UI (FR-10), /health. **Главный экран `GET /` — матрица «репозиторий × артефакт»** (D7→D14 #58, решение CEO 2026-07-30 по фокус-группам): тихие подложки статусов + чип разрыва с уверенностью (D13), плашка «сдача через MR» (D9), сигнальные блоки FR-6/7/3 и флаг устаревшего обхода (D10), честный свод ячейки (D11), легенда (D12), модальное окно деталей (файлы, рёбра с цитатами, FR-10). Матрица «репозиторий × занятие» — вспомогательный вид на `GET /lessons`. Ядро FR-5 реализовано (гейт Фазы 0 снят 2026-07-30, ADR-004 Accepted): `llm_client` (C1) + `coherence_analyzer` (C2), воркер вердиктов подключён к своду через /sync; без ключа DeepSeek пары остаются в состоянии «проверяется». Трекер: [GitHub Issues](https://github.com/genarovv/course-dashboard/issues), карта тестов: [tests/MAP.md](tests/MAP.md).
