...
def _count_artifacts(artifact_defs: list[ArtifactDef], tree: list[str]) -> int:
    """Сколько ролей курса представлено в дереве — мера «здесь есть работа».
    ...
    Роль засчитывается и по контрактному пути, и по кандидату «не там» (D44).
    Иначе мера бесполезна ровно там, ради чего писалась: студент, у которого
    работа лежит в другой ветке, почти всегда и путей контракта не соблюдает —
    ветка с тридцатью файлами работы давала ровно тот же счёт, что пустой
    шаблон (проверено живым прогоном на репозиториях потока 2026-08-11).
    """
    roles = set()
    for adef in artifact_defs:
        if adef.role in roles:
            continue
        if _match_artifact(adef, tree) or _displaced_candidates(
            adef.expected_pattern, tree, set()
        ):
            roles.add(adef.role)
    return len(roles)
